...
function updateUIForState() {
 ...
 switch (currentState) {
   ...
   case STATES.CONFIGURED:
    ...
    openSavedAssaysButton.hidden = true; 
    break;
...
function updateProgressTable() {
  ...
  trial.runs.forEach(run => {
    if (!summary[run.genotype]) return;

    summary[run.genotype].total++;

    if (run.status === "completed" && run.eligibleForAnalysis) {
      summary[run.genotype].eligible++;
    } else if (run.status !== "active") {
      summary[run.genotype].ineligible++;
    }
  });
...
async function abandonAllActiveTrialsInDB() {
  const db = await openDB();

  const tx = db.transaction(STORES.TRIALS, "readwrite");
  const index = tx.objectStore(STORES.TRIALS).index("status");
  const req = index.getAll("active");

  req.onsuccess = () => {
    req.result.forEach(trial => {
      trial.status = "abandoned";
